...
class TestCaseSecurity(TransactionCase):
    ...
    def setUp(self):
        ...
        self.document_user = self.env['res.users'].create({
            ...
            'groups_id': [(6, 0, [self.ref('viin_document.viin_document_group_user')])]
        })
        ...
        self.document_manager = self.env['res.users'].create({
            ...
            'groups_id': [(6, 0, [self.ref('viin_document.viin_document_group_manager')])]
        })
    ...
    def test_documents_access_default(self):
        ...
        folder_d = self.env['viin_document.folder'].create({
            'name': 'folder D',
        })
        document_d = self.env['viin_document.document'].create({
            'name': 'document D',
    ...
    def test_documents_access_manager_read_write(self):
        """
        Tests the access rights for a document where viin_document.viin_document_group_manager is the only group with access (read/write).
        """
    ...
    def test_documents_access_arbitrary_readonly(self):
        ...
        folder_b = self.env['viin_document.folder'].create({
            'name': 'folder B',
            'read_group_ids': [(6, 0, [self.arbitrary_group.id])],
        })
        document_b = self.env['viin_document.document'].create({
            'name': 'document B',
    ...
    def test_documents_arbitrary_read_write(self):
        ...
        The viin_document.viin_document_group_manager always keeps the read/write access.
        """
    ...
    def test_documents_access(self):
        ...
        document_owner = self.env['viin_document.document'].create({
            'name': 'document owner',
            'folder_id': folder_owner.id,
            'owner_id': self.test_group_user.id,
        })
        document_owner2 = self.env['viin_document.document'].create({
            'name': 'document owner2',
            'folder_id': folder_owner.id,
            'owner_id': self.test_group_user2.id,
        })
        document_not_owner = self.env['viin_document.document'].create({
            'name': 'document not owner',
            'folder_id': folder_owner.id,
        })
        document_read_owner = self.env['viin_document.document'].create({
        ...
        document_from_user = self.env['viin_document.document'].with_user(self.test_group_user).browse(
    ...
    def test_share_link_access(self):
        ...
        folder_share = self.env['viin_document.folder'].create({
            'name': 'folder share',
        })
    ...
    def test_share_link_dynamic_access(self):
        ...
        folder_share = self.env['viin_document.folder'].create({
            'name': 'folder share',
            'read_group_ids': [(6, 0, [self.ref('viin_document.viin_document_group_user')])]
        })
        document_a = self.env['viin_document.document'].create({
            'datas': b"R0lGODdhAQABAIAAAP///////ywAAAAAAQABAAACAkQBADs=",
