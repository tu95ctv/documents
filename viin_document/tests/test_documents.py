...
class TestCaseDocuments(TransactionCase):
    ...
    def setUp(self):
        ...
        self.folder_a = self.env['viin_document.folder'].create({
            'name': 'folder A',
        })
        self.folder_a_a = self.env['viin_document.folder'].create({
            'name': 'folder A - A',
            'parent_folder_id': self.folder_a.id,
        })
        self.folder_b = self.env['viin_document.folder'].create({
            'name': 'folder B',
        })
        self.tag_category_b = self.env['viin_document.tag.cate'].create({
        ...
        self.tag_category_a_a = self.env['viin_document.tag.cate'].create({
        ...
        self.document_txt = self.env['viin_document.document'].create({
            'datas': TEXT,
            'name': 'file.txt',
            'mimetype': 'text/plain',
            'folder_id': self.folder_b.id,
        ...
        self.worflow_rule = self.env['viin_document.action'].create({
            ...
            'activity_type_id': self.env.ref('viin_document.mail_documents_activity_data_Inbox').id,
    ...
    # def test_documents_create_from_attachment(self):#này tạm bỏ
    #     print ('test_documents_create_from_attachment')
    #     """
    #     Tests a viin_document.document create method when created from an already existing ir.attachment.
    #     """
    ...
    #     self.assertEqual(document_a.res_model, 'viin_document.document',
    ...
    def test_documents_create_write(self):
        """
        Tests a viin_document.document create and write method,
        ...
        #việc ghi của document_no_attachment
        document_no_attachment = self.env['viin_document.document'].create({
            'name': 'Test mimetype gif',
            'folder_id': self.folder_b.id,
        })
        ...
        self.assertEqual(document_no_attachment.attachment_id.datas, TEXT, 'the document should have an attachment')#tạch
    #rule bỏ chưa test
    def test_documents_rules(self):
        """
        Tests a viin_document.action
        """
        ...
        self.assertEqual(activity_gif.activity_type_id.id,
                         self.env.ref('viin_document.mail_documents_activity_data_Inbox').id,
                         "failed at activity data note from workflow create activity")
    ...
    def test_documents_rule_display(self):
        ...
        self.workflow_rule_criteria = self.env['viin_document.action'].create({
        ...
        self.document_txt_criteria_a = self.env['viin_document.document'].create({
            'name': 'Test criteria a',
            'mimetype': 'text/plain',
            'folder_id': self.folder_a.id,
            'tag_ids': [(6, 0, [self.tag_a_a.id, self.tag_b.id])]
        })
        # chưa test available_rule_ids
        ...
        self.document_txt_criteria_b = self.env['viin_document.document'].create({
        ...
        self.assertTrue(self.workflow_rule_criteria.id not in self.document_txt_criteria_b.available_rule_ids.ids,
                        "failed at documents_workflow_rule unavailable rule")
        self.document_txt_criteria_c = self.env['viin_document.document'].create({
            'datas': TEXT,
            'name': 'file.txt',
            'mimetype': 'text/plain',
            'folder_id': self.folder_a.id,
        ...
        self.document_txt_criteria_d = self.env['viin_document.document'].create({
    ...
    def test_viin_document_share_links(self):
        ...
        vals = {
            ...
            'activity_option': True,
            'activity_type_id': self.ref('viin_document.mail_documents_activity_data_tv'),
    ...
    def test_request_activity(self):
        ...
        document_1 = self.env['viin_document.document'].search([('request_activity_id', '=', activity.id)], limit=1)
        document_2 = self.env['viin_document.document'].search([('request_activity_id', '=', activity_2.id)], limit=1)
    ...
    def test_default_res_id_model(self):#tạch
        """
        Test default res_id and res_model from context are used for linking attachment to document.
        """
        document = self.env['viin_document.document'].create({'folder_id': self.folder_b.id})
        ...
        # self.assertEqual(document.attachment_id, attachment, "Document should be linked to the created attachment")#tạch đã bỏ việc tạo attachment trước

    # def test_versioning(self):
    #     """
    #     Tests the versioning/history of documents
    #     """
    #     document = self.env['viin_document.document'].create({'name':'gif','datas': GIF, 'folder_id': self.folder_b.id})
    ...
    def test_write_mimetype(self):
        ...
        self.assertEqual(document.mimetype, 'text/plain', "the new mimetype should be the one given on write: old: %s")
    ...
    def test_cascade_delete(self):
        ...
        """
        document = self.env['viin_document.document'].create({'name':'a','datas': GIF, 'folder_id': self.folder_b.id})
    ...
    #     user = new_test_user(self.env, "test user", groups='viin_document.viin_document_group_user')
    #     document = self.env['viin_document.document'].create({'datas': GIF, 'folder_id': self.folder_b.id})
